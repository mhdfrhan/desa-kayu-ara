...
@section('page-title', 'Edit Profil Kampung')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Profil Kampung</h1>
                <p class="mt-1 text-sm text-gray-600">Edit konten profil kampung yang akan ditampilkan di halaman publik</p>
            </div>
                ...
                <div class="px-6 py-4 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="jenis" id="jenis" required
                                ...
                                <option value="visi" {{ old('jenis', $profil->jenis) == 'visi' ? 'selected' : '' }}>Visi
                                </option>
                                <option value="misi" {{ old('jenis', $profil->jenis) == 'misi' ? 'selected' : '' }}>Misi
                                </option>
                                <option value="sejarah" {{ old('jenis', $profil->jenis) == 'sejarah' ? 'selected' : '' }}>
                                    Sejarah</option>
                                <option value="struktur" {{ old('jenis', $profil->jenis) == 'struktur' ? 'selected' : '' }}>
                                    Struktur Organisasi</option>
                                <option value="lokasi" {{ old('jenis', $profil->jenis) == 'lokasi' ? 'selected' : '' }}>
                                    Lokasi</option>
                                <option value="potensi" {{ old('jenis', $profil->jenis) == 'potensi' ? 'selected' : '' }}>
                                    Potensi Kampung</option>
                                <option value="demografi"
                                    {{ old('jenis', $profil->jenis) == 'demografi' ? 'selected' : '' }}>Demografi</option>
                                <option value="infrastruktur"
                                    {{ old('jenis', $profil->jenis) == 'infrastruktur' ? 'selected' : '' }}>Infrastruktur
                                </option>
                            </select>
                            @error('jenis')
                        ...
                        <div>
                            ...
                            <input type="text" name="judul" id="judul" value="{{ old('judul', $profil->judul) }}"
                    ...
                    <div>
                        ...
                        <textarea name="konten" id="konten" rows="8"
                            ...
                            placeholder="Masukkan konten profil kampung yang lengkap dan informatif">{{ old('konten', $profil->konten) }}</textarea>
                        @error('konten')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div id="imagePreview" class="{{ $profil->gambar ? '' : 'hidden' }}">
                                <img id="preview" src="{{ $profil->gambar ? asset($profil->gambar) : '' }}"
                                    alt="Preview" class="w-20 h-20 object-cover rounded-lg border border-gray-300">
                            </div>
                        </div>
                        @if ($profil->gambar)
                            <div class="mt-2">
                                <p class="text-xs text-gray-500">Gambar saat ini: {{ basename($profil->gambar) }}</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="urutan" id="urutan"
                                value="{{ old('urutan', $profil->urutan) }}" min="0"
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center">
                                <input type="checkbox" name="aktif" id="aktif" value="1"
                                    {{ old('aktif', $profil->aktif) ? 'checked' : '' }}
                                ...
                                foreach ($misiLines as $index => $line) {
                                    ...
                                    if (!empty($trimmedLine)) {
                                        if (preg_match('/^\d+\./', $trimmedLine)) {
                                            $numberedLines[] = $trimmedLine;
                                        } else {
                                            $numberedLines[] = $index + 1 . '. ' . $trimmedLine;
                                        }
                                    }
                                }
                            @endphp
                            {!! implode('<br>', $numberedLines) !!}
                        @else
